...
/* Private variables ---------------------------------------------------------*/
extern struct sensor1_t sensor_data;
char rx_buffer[4];
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 2 */
   // HAL_UART_Receive(&huart1, rx_buffer, 4, 100);
...
__weak void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
   ...
   */
//  if(rx_buffer[0] == 0xFF && rx_buffer[3] == ((rx_buffer[0]+rx_buffer[1]+rx_buffer[2])&0xFF))
 // 	{
  //		uint16_t distance_measured = (rx_buffer[1]*256)+ rx_buffer[2];
  //		PRINTF("distance_measured = %d\n", distance_measured);
  //	}
  //	HAL_UART_Receive_DMA(&huart1, (uint8_t *)rx_buffer, 4);
